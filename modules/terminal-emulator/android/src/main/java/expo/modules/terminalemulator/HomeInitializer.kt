...
""".trimIndent()
    ...
    fun initialize(context: Context): File {
        ...
        if (!bashrc.exists() || currentVersion < BASHRC_VERSION) {
            ...
            sb.appendLine("__shelly_mkdir -p \"\$CLAUDE_CODE_TMPDIR\" 2>/dev/null")
            ...
            sb.appendLine("__shelly_mkdir -p \"\$BUN_TMPDIR\" 2>/dev/null")
            ...
            sb.appendLine("  __shelly_mkdir -p \"\$HOME/.shelly-cli\" 2>/dev/null || true")
            ...
            sb.appendLine("  __shelly_mkdir -p \"\$__shelly_tmp\" \"\$__bun_tmp\" \"\$__claude_tmp\" \"\$HOME/.config\" \"\$HOME/.cache\" \"\$HOME/.local/share\" 2>/dev/null || true")
            sb.appendLine("  /system/bin/env -i \\")
            sb.appendLine("    HOME=\"\$HOME\" PWD=\"\$PWD\" USER=\"\${USER:-shelly}\" LOGNAME=\"\${LOGNAME:-shelly}\" SHELL=\"\$SHELL\" \\")
            sb.appendLine("    TERM=\"\${TERM:-xterm-256color}\" COLORTERM=\"\${COLORTERM:-truecolor}\" LANG=\"\${LANG:-C.UTF-8}\" LC_ALL=\"\${LC_ALL:-C.UTF-8}\" \\")
            sb.appendLine("    PATH=\"\$PATH\" LD_LIBRARY_PATH=\"\$SHELLY_LD_LIBRARY_PATH\" ANDROID_DATA=\"\${ANDROID_DATA:-/data}\" ANDROID_ROOT=\"\${ANDROID_ROOT:-/system}\" \\")
            ...
            sb.appendLine("__shelly_cat > \"\$__shelly_gemini_raw_mode_preload\" <<'__SHELLY_GEMINI_RAW_MODE_PRELOAD__'")
            sb.appendLine("(function() {")
            ...
            sb.appendLine("__shelly_chmod 600 \"\$__shelly_gemini_raw_mode_preload\" 2>/dev/null || true")
            sb.appendLine("__shelly_claude_node_preload=\"\$HOME/.shelly-claude-node-preload.js\"")
            sb.appendLine("__shelly_cat > \"\$__shelly_claude_node_preload\" <<'__SHELLY_CLAUDE_NODE_PRELOAD__'")
            ...
            sb.appendLine("      return [\"'\" + shellPath + \"'\", '\"' + shellPath + '\"', shellPath].flatMap(function(token) {")
            sb.appendLine("        const matches = [];")
            sb.appendLine("        let from = 0;")
            sb.appendLine("        while (from < tail.length) {")
            sb.appendLine("          const index = tail.indexOf(token, from);")
            sb.appendLine("          if (index < 0) break;")
            sb.appendLine("          matches.push({ token, shellPath, index });")
            sb.appendLine("          from = index + token.length;")
            sb.appendLine("        }")
            sb.appendLine("        return matches;")
            sb.appendLine("      });")
            sb.appendLine("    })")
            sb.appendLine("      .filter(function(item) {")
            ...
            sb.appendLine("    const commandPrefix = '/system/bin/linker64 ';")
            sb.appendLine("    const absoluteIndex = marker + picked.index;")
            sb.appendLine("    const prefix = arg.slice(0, absoluteIndex);")
            sb.appendLine("    if (prefix.slice(-(inject.length + commandPrefix.length)) === inject + commandPrefix) {")
            ...
            sb.appendLine("      shellyPatchTrace(source + ' fired token=' + JSON.stringify(picked.token) + ' index=' + absoluteIndex + ' linker=1 existing-env=1');")
            sb.appendLine("      return prefix + commandPrefix + arg.slice(absoluteIndex);")
            sb.appendLine("    }")
            sb.appendLine("    shellyPatchTrace(source + ' fired token=' + JSON.stringify(picked.token) + ' index=' + absoluteIndex + ' linker=1');")
            sb.appendLine("    return prefix + inject + commandPrefix + arg.slice(absoluteIndex);")
            sb.appendLine("  };")
            ...
            sb.appendLine("__shelly_chmod 600 \"\$__shelly_claude_node_preload\" 2>/dev/null || true")
            ...
            sb.appendLine("  __shelly_mkdir -p \"\$HOME/.claude\" 2>/dev/null || true")
            ...
            sb.appendLine("        __shelly_paste_tui_begin")
            sb.appendLine("        SHELLY_LIB_DIR=\"$libDir\" SHELLY_CODEX_EXEC_PATH=\"\$__chosen_exec\" SHELLY_CODEX_PROC_EXE_SHIM=1 SHELLY_CODEX_PROC_EXE_OPEN_SHIM=1 LD_PRELOAD=\"$libDir/libexec_wrapper.so\" _run \"\$__chosen_exec\" \"\${__codex_args[@]}\"")
            ...
            sb.appendLine("    __shelly_paste_tui_begin")
            sb.appendLine("    SHELLY_LIB_DIR=\"$libDir\" SHELLY_CODEX_EXEC_PATH=\"\$__chosen_tui\" SHELLY_CODEX_PROC_EXE_SHIM=1 SHELLY_CODEX_PROC_EXE_OPEN_SHIM=1 LD_PRELOAD=\"$libDir/libexec_wrapper.so\" _run \"\$__chosen_tui\" \"\${__codex_args[@]}\"")
            ...
            sb.appendLine("    __out=\$(HOME=\"\$HOME\" CODEX_HOME=\"\${CODEX_HOME:-\$HOME/.codex}\" TERM=\"\${TERM:-xterm-256color}\" TMPDIR=\"\${TMPDIR:-\$HOME/.tmp}\" PATH=\"\$HOME/bin:$libDir:\${PATH:-/system/bin:/vendor/bin}\" timeout 120 /system/bin/env SHELLY_CODEX_EXEC_PATH=\"\$__chosen_exec\" SHELLY_CODEX_PROC_EXE_SHIM=1 SHELLY_CODEX_PROC_EXE_OPEN_SHIM=1 LD_PRELOAD=\"$libDir/libexec_wrapper.so\" LD_LIBRARY_PATH=\"\$SHELLY_LD_LIBRARY_PATH\" /system/bin/linker64 \"\$__chosen_exec\" \"\$__prompt\" 2>&1)")
            sb.appendLine("    __rc=\$?")
            sb.appendLine("    printf '%s\\n' \"\$__out\" | sed -n '1,120p'")
            ...
            sb.appendLine("    __out=\$(cd \"\$__work\" && HOME=\"\$HOME\" CODEX_HOME=\"\${CODEX_HOME:-\$HOME/.codex}\" TERM=\"\${TERM:-xterm-256color}\" TMPDIR=\"\${TMPDIR:-\$HOME/.tmp}\" PATH=\"\$HOME/bin:$libDir:\${PATH:-/system/bin:/vendor/bin}\" timeout 180 /system/bin/env SHELLY_CODEX_EXEC_PATH=\"\$__chosen_exec\" SHELLY_CODEX_PROC_EXE_SHIM=1 SHELLY_CODEX_PROC_EXE_OPEN_SHIM=1 LD_PRELOAD=\"$libDir/libexec_wrapper.so\" LD_LIBRARY_PATH=\"\$SHELLY_LD_LIBRARY_PATH\" /system/bin/linker64 \"\$__chosen_exec\" \"\$__prompt\" 2>&1)")
            sb.appendLine("    __rc=\$?")
            sb.appendLine("    printf '%s\\n' \"\$__out\" | sed -n '1,160p'")
            ...
            sb.appendLine("    __out=\$(cd \"\$__work\" && HOME=\"\$HOME\" CODEX_HOME=\"\${CODEX_HOME:-\$HOME/.codex}\" TERM=\"\${TERM:-xterm-256color}\" TMPDIR=\"\${TMPDIR:-\$HOME/.tmp}\" PATH=\"\$HOME/bin:$libDir:\${PATH:-/system/bin:/vendor/bin}\" timeout 240 /system/bin/env SHELLY_CODEX_EXEC_PATH=\"\$__chosen_exec\" SHELLY_CODEX_PROC_EXE_SHIM=1 SHELLY_CODEX_PROC_EXE_OPEN_SHIM=1 LD_PRELOAD=\"$libDir/libexec_wrapper.so\" LD_LIBRARY_PATH=\"\$SHELLY_LD_LIBRARY_PATH\" /system/bin/linker64 \"\$__chosen_exec\" \"\$__prompt\" 2>&1)")
            sb.appendLine("    __rc=\$?")
            sb.appendLine("    printf '%s\\n' \"\$__out\" | sed -n '1,180p'")
            ...
            sb.appendLine("}")
            sb.appendLine("shelly-claude-bash-trace() {")
            sb.appendLine("  local __runtime_cli=\"\$HOME/.shelly-runtime/claude-extracted/current/node_modules/@anthropic-ai/claude-code-extracted/cli.js\"")
            sb.appendLine("  local __apk_cli=\"$libDir/node_modules/@anthropic-ai/claude-code-extracted/cli.js\"")
            ...
            sb.appendLine("  local __node_options=\"--require=\$__shelly_node_compat_preload --require=\$__shelly_claude_node_preload\"")
            sb.appendLine("  local __log=\"\$HOME/.shelly-claude-patch.log\"")
            sb.appendLine("  local __nonce=\"SHELLY_TRACE_\$(date +%s)_\$\$\"")
            sb.appendLine("  rm -f \"\$__log\" 2>/dev/null || true")
            sb.appendLine("  echo \"trace_nonce=\$__nonce\"")
            sb.appendLine("  USE_BUILTIN_RIPGREP=0 DISABLE_AUTOUPDATER=1 DISABLE_INSTALLATION_CHECKS=1 NO_UPDATE_NOTIFIER=1 CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=0 SHELL=\"\$HOME/bin/bash\" BASH=\"\$HOME/bin/bash\" SHELLY_LIB_DIR=\"$libDir\" PATH=\"\$HOME/bin:$libDir:\${PATH:-/system/bin:/vendor/bin}\" LD_LIBRARY_PATH=\"$libDir\" TMPDIR=\"\${TMPDIR:-\$HOME/.tmp}\" BUN_TMPDIR=\"\${BUN_TMPDIR:-\$HOME/.bun-tmp}\" CLAUDE_CODE_TMPDIR=\"\${CLAUDE_CODE_TMPDIR:-\$HOME/.claude-tmp}\" CLAUDE_TMPDIR=\"\${CLAUDE_TMPDIR:-\$HOME/.claude-tmp}\" NODE_OPTIONS=\"\$__node_options\" LD_PRELOAD=\"$libDir/libexec_wrapper.so\" SHELLY_CLAUDE_DIAG=1 SHELLY_CLAUDE_PATCH_TRACE=1 SHELLY_CLAUDE_NATIVE_TRACE=1 SHELLY_CLAUDE_CANARY_TRACE=1 SHELLY_CLAUDE_PATCH_RAW=\"\$__raw\" BASH_ENV= ENV= SHELLY_BASH_CANARY_NONCE=\"\$__nonce\" timeout 45 /system/bin/env LD_LIBRARY_PATH=\"\$SHELLY_LD_LIBRARY_PATH\" /system/bin/linker64 $libDir/node \"\$__cli\" --print --allowedTools Bash --tools Bash --permission-mode bypassPermissions 'Use the Bash tool to run this exact command: printf \"\$SHELLY_BASH_CANARY_NONCE\". Return only the command output.' 2>&1 | sed -n '1,80p'")
            sb.appendLine("  local __rc=\${PIPESTATUS[0]}")
            sb.appendLine("  echo \"trace_rc=\$__rc\"")
            sb.appendLine("  echo \"trace_log=\$__log\"")
            sb.appendLine("  if [ -f \"\$__log\" ]; then tail -120 \"\$__log\"; else echo 'trace_log_missing'; fi")
