...
""".trimIndent()
    ...
    private const val BASHRC_VERSION = 214
    ...
    fun initialize(context: Context): File {
        ...
        if (!bashrc.exists() || currentVersion < BASHRC_VERSION) {
            ...
            sb.appendLine("    __out=\$(cd \"\$__work_canon\" && HOME=\"\$HOME\" CODEX_HOME=\"\${CODEX_HOME:-\$HOME/.codex}\" TERM=\"\${TERM:-xterm-256color}\" TMPDIR=\"\${TMPDIR:-\$HOME/.tmp}\" PATH=\"\$HOME/bin:$libDir:\${PATH:-/system/bin:/vendor/bin}\" timeout 240 /system/bin/env SHELLY_CODEX_EXEC_PATH=\"\$__chosen_exec\" SHELLY_CODEX_PROC_EXE_SHIM=1 SHELLY_CODEX_PROC_EXE_OPEN_SHIM=1 LD_PRELOAD=\"$libDir/libexec_wrapper.so\" LD_LIBRARY_PATH=\"\$SHELLY_LD_LIBRARY_PATH\" /system/bin/linker64 \"\$__chosen_exec\" --cd \"\$__work_canon\" --add-dir \"\$__work_canon\" --add-dir \"\$__work\" --add-dir \"\$__work_alias\" --add-dir \"\$__work_canon_alias\" --skip-git-repo-check \"\$__prompt\" 2>&1)")
            sb.appendLine("    __rc=\$?")
            sb.appendLine("    printf '%s\\n' \"\$__out\" | sed -n '1,220p'")
