...
- **Local LLM support** — use the built-in GGUF catalog and llama.cpp / llama-server controls, then route via `@local` for fully on-device inference. The high-end recommendation is Qwen3.5-4B Q4_K_M; Qwen 2.5 1.5B remains the low-memory fallback.
...
- **No offline mode by default** — Cloud AI features require an internet connection. Local LLM via `@local` works offline with the bundled catalog and llama.cpp / llama-server controls; Qwen3.5-4B Q4_K_M is recommended for high-end foldables, while Qwen 2.5 1.5B is the low-memory option.
...
- **Local LLM mode** — For fully private usage, configure a local GGUF model through llama.cpp. Qwen3.5-4B Q4_K_M is the recommended high-end model; Qwen 2.5 1.5B is available for low-memory devices. All processing stays on-device.
